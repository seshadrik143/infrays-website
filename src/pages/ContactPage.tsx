...
export default function ContactPage() {
  ...
  function reset() {
    setName(''); setEmail(''); setMessage(''); setTopic(topics[0]); setStatus('idle')
  }
          ...
          <div className="container-md">
            <div className="grid md:grid-cols-5 gap-10">
              ...
              <div className="md:col-span-3">
                ...
                {status === 'sent' ? (
                  <div className="border border-green-500/20 rounded-2xl p-10 flex flex-col items-center justify-center text-center space-y-5"
                    style={{ background: 'rgba(16,185,129,0.04)', minHeight: '360px' }}>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/45 max-w-xs mx-auto leading-relaxed">
                        Thanks, <span className="text-white/70">{name}</span>. We'll get back to you at{' '}
                        <span className="text-cyan-400">{email}</span> within 24 hours.
                      </p>
                    </div>
                  ...
                  <form onSubmit={handleSend}
                    ...
                    {status === 'error' && (
                      <div className="border border-red-500/20 rounded-xl p-4 flex items-start gap-3"
                        style={{ background: 'rgba(239,68,68,0.04)' }}>
                        <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
                        <p className="text-xs text-white/50">
                          Couldn't send — email us directly at{' '}
                          <span className="text-cyan-400">{SALES_EMAIL}</span>
                        </p>
                      </div>
                    )}
